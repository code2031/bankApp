...
export default class validations {
  ...
  static async signupValidations(body) {
    ...
    if (!middleName || middleName.length < 3 || !validName.test(middleName)) {
      ...
      signupErrors.middleName.push('Middle name is required, with at least three alphabetical characters');
    }

    if (!phoneNumber || !validPhoneNumber.test(phoneNumber)) {
      signupErrors.phoneNumber = [];
      signupErrors.phoneNumber.push('Phone Number is required and must be up to 11 digits');
    }
